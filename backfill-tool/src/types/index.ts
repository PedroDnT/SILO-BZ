...
export interface CVMRecord {
    ...
    doc_type: string;
    cnpj_key: string;
    competence_date: string; // ISO 8601 format
    payload: JsonObject;
}
...
export interface SGSObservation {
    series_code: number;
    obs_date: string; // ISO 8601 format
    value: number | null;
}
...
export interface PTAXRate {
    ...
    rate_datetime: string; // ISO 8601 format
    bid: number;
    ask: number;
}
...
export interface FetchConfig {
    ...
    maxRetries: number;
    retryBackoff: number[]; // milliseconds for each retry attempt
}
