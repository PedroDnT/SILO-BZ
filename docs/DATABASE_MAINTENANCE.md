...
| `run_daily` exits 1 on `etf_market` only; log shows HTTP 403 `platform-feature-disabled` / `Monthly usage hard limit exceeded` | Apify account hit its monthly usage cap (Daily CVM Ingest #220/#221, runs 34089192930 / 34193021487, 2026-09-07/08). The actor never started. | Maps to `ApifyUsageLimitError` and skips like an unset token so ANALYZE + analytical still run. Raise or wait out the Apify plan if snapshots are needed. Other 403s (for example `billing`) still fail the daily run. |
