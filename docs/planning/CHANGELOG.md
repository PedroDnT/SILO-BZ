...
| Date       | Branch                    | Change                                                                                              |
| ---------- | ------------------------- | --------------------------------------------------------------------------------------------------- |
| 2026-08-28 | cursor/warehouse-health-gate-652a | Health gate: catalog key is `version` (not `catalog_version`); ingest errors count unresolved slices not recovered TimeoutError attempts; diagnostics no longer `ON_ERROR_STOP=1` (missing `vw_b3_share_count_event` must not abort the job); disk 899% of Pro 8 GB is a warn + ops note, not a drop-tables fix |
