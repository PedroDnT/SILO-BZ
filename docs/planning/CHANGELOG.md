...
| 2026-05-29 | feat/cia-cadastre-ipe (W6) | CIA_ABERTA cadastre + IPE ingest: field maps cia_company.py (CAD, verified live) + cia_event.py (IPE, verified live), src/pipeline/ingest_cia.py (ingest_cia_company, ingest_cia_event with drop-row rules), wired ingest_cia_cad/ingest_cia_ipe into CVMIngestor backfill + daily_update (cad once per run, ipe per year from 2010), 15 new unit tests in tests/test_cia_field_maps.py |
