...
| `cvm_fi_balancete`       | fund × month × account               | monthly ZIP                          | 2019                     |
...
| `cvm_fidc_aging`                                                                               | receivables by aging bucket per FIDC                                                                                                             | **No.** Read by `/fidc` and `fraud_screen_evergreen_aging` (which reaches the API as the signal `screen_evergreen_aging`, not the buckets). Candidate: `fidc_aging(p_cnpj, …)`, same shape argument as tranches. `panel`'s `delinquency` metric is the total only.                                                                                                                                                                                                                                                                                                                                                |
