...
| `fnet_document_body`, `fnet_document_pair`, `fnet_document_diff` | FNET restatement diffs: the parsed shape of a re-filed FIDC informe mensal and the fields that differ from the version it replaced (migration 46; bodies not kept) | **Yes** (catalog v40, `27_api_fnet_diff.sql`): `fund_restatement_diff`, plus `fund_restatements.diff_status` / `n_fields_changed`; `coverage()` row `fnet_restatement_diffs`. Slice 1 is FIDC informe mensal estruturado only. |
...
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bacen_expectativas` | the Focus survey | **No.** Ingested daily and read only by the `/macro` dashboard page. The IPCA set of `bacen_sgs` is served by `api.inflation` (v30) and the rest of `bacen_sgs` plus `bacen_ptax` by `api.macro_series` / `api.ptax` (v38) — see the served table above. Candidate: the Focus survey needs its own shape (indicator × horizon × statistic). |
