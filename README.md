...
| B3 | **BDI** — securities lending, investor flow, index float, instrument registry | short balance and borrow rates per ticker; buy/sell volume per investor type; free-float share counts and B3 sector; shares outstanding per ticker | daily — **~21 business days of retention, no archive** | the Short Monitor (% of float, days to cover, borrow cost) and the investor-flow panel. **Not backfillable**: a session missed is lost, so the daily job is the only way this history exists |
