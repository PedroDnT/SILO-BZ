...
Coverage figures are as of 2026-08-31. [`docs/DATABASE_MAINTENANCE.md`](https://github.com/PedroDnT/SILO-BZ/blob/main/docs/DATABASE_MAINTENANCE.md) §11 keeps
...
The monthly FIDC ZIP has 18 members; ten are ingested after migration 38 (`IV`,
`VI`, `X_2`/`X_3`/`X_6`, `X_4`, and `I` cedente slots, `II`, `VIII`, `X`). Still
unread: tab I's other ~70 columns (asset composition, admin, condomínio), `III`
(2025+), `V` (maturity ladder of credits with risk retention), `VII` (custody
split, repurchases), `IX` (assignment prices), `X_1` / `X_1_1` (quotaholders per
tranche and by investor type), `X_5` (liquidity ladder), `X_7` (collateral
coverage).
...
Called the highest-value additions in [`docs/planning/INSTRUMENTS.md`](https://github.com/PedroDnT/SILO-BZ/blob/main/docs/planning/INSTRUMENTS.md). Each is a
...
| `cvm_fidc_cedente`, `cvm_fidc_sacado`, `cvm_fidc_setor`, `cvm_fidc_scr` | FIDC informe tabs I, VIII, II, X (migration 38) | **Yes**: `fidc_cedentes` (by fund or by originator CPF/CNPJ/ticker, `cedente_tickers` from the FCA map), `fidc_sacados` (anonymized ranks, by fund only), `fidc_portfolio` (sector hierarchy with `parent`, SCR ladders, tax debt — long), the panel metrics `receivables` / `sacado_top1` / `sacado_top25`, and four `coverage()` rows. See [FIDC concentration](/api-docs/fidc). |
