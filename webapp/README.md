...
`3.09` is profit _before_ the statutory profit-sharing on `3.10`, so it equals
...
what the codes _mean_ (`3.01` is interest income, `3.05` is pre-tax profit), so
...
**Built, not deployed.** No public URL serves this site, `/growth` included:
the two dashboard hosts serve `dashboard/`, not this directory. The open item is
