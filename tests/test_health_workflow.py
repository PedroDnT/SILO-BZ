...
# ── the completeness gate must compare families on one scale ──────────────


def _completeness_block() -> str:
    """The check-3 body, from the health job's inline script."""
    text = WORKFLOW.read_text(encoding="utf-8")
    return text.split("# 3. Completeness drift")[1].split("# 4.")[0]


def test_completeness_age_is_measured_from_month_end_not_the_period_key():
    """The families disagree on where in the month the period key sits.

    fii and fiagro key the FIRST (2026-07-01), fidc the LAST (2026-07-31), so
    `CURRENT_DATE - complete_through` charges a month-start family ~30 extra
    days for the identical filing state. Run #41 failed on exactly that:

        fidc    664/4253 = 0.156   -> 48d  PASS
        fii     247/1292 = 0.191   -> 78d  FAIL
        fiagro   75/ 254 = 0.295   -> 78d  FAIL

    fidc was the worst filed of the three and passed. Normalising to month-end
    puts all three at 48d, which is the reality they actually share.
    """
    block = _completeness_block()
    assert "date_trunc('month'" in block, "age must be normalised to the period's month"
    assert "1 month - 1 day" in block, "normalise to month END, not month start"
    # The raw difference must not survive anywhere in the gate or its diagnostic.
    for line in block.splitlines():
        code = line.split("#", 1)[0]
        if "CURRENT_DATE - public.latest_complete_period" in code.replace(" ", " "):
            raise AssertionError(f"raw key difference still used: {line.strip()}")


def test_the_gate_and_its_diagnostic_measure_the_same_thing():
    """An operator reading days_behind must see the number the gate judged.

    The printed table and the failing predicate drifting apart is how a red
    run becomes unexplainable from its own log.
    """
    block = _completeness_block()
    normalised = block.count(
        "date_trunc('month', public.latest_complete_period(entity_type))::date"
    )
    assert normalised == 2, (
        f"expected the diagnostic and the gate to share the expression, found {normalised}"
    )


def test_fip_stays_excluded_because_it_files_yearly():
    """fip is keyed 31-December, so its complete period is up to a year back.

    Verified 2026-09-17: complete_through 2025-12-31, newest_period 2026-12-31,
    landed_at today. A days-behind threshold cannot judge it at all — month-end
    normalisation does not rescue a yearly family, so the exclusion stays.
    """
    assert "entity_type <> 'fip'" in _completeness_block()
