...
AS $$
    ...
    FROM public.anbima_class_monthly a
    UNION ALL
    -- FIDC concentration tabs (migration 38). Members of the same monthly
    -- informe as cvm_fidc_mensal, so the fidc completeness clamp applies.
    -- Each MAX is an index-only probe on (period DESC). notes carry what the
    -- dates cannot: where each series starts and what its identifiers are.
    SELECT 'fidc_cedentes'::text, MAX(c.period), public.latest_complete_period('fidc'), 'cvm'::text,
    ...
    FROM public.cvm_fidc_cedente c
    UNION ALL
    SELECT 'fidc_sacados'::text, MAX(k.period), public.latest_complete_period('fidc'), 'cvm'::text,
           'tab VIII from 2013-01: the 25 largest debtors as anonymized (rank, value); seq is CVM''s rank as filed, never recomputed'::text
    FROM public.cvm_fidc_sacado k
    UNION ALL
    SELECT 'fidc_sectors'::text, MAX(s.period), public.latest_complete_period('fidc'), 'cvm'::text,
           'tab II from 2013-01: receivables by sector, a hierarchy (fidc_portfolio.parent); TOTAL is the panel metric receivables'::text
    FROM public.cvm_fidc_setor s
    UNION ALL
    SELECT 'fidc_scr'::text, MAX(r.period), public.latest_complete_period('fidc'), 'cvm'::text,
