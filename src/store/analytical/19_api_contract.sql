...
AS $$
    ...
    base AS (
        ...
        UNION ALL
        -- v40 (B4 slice 1, 27_api_fnet_diff.sql). The restatement-diff queue:
        -- the period is the delivery day of the newest re-filing that has a
        -- compared pair. complete_through is NULL: the queue is worked
        -- newest-first under a per-run cap, so no delivery day is ever
        -- "complete" — fund_restatements.diff_status says, row by row,
        -- whether a re-filing has been looked at. landed_at is the fnet /
        -- diff audit row.
        SELECT 'fnet_restatement_diffs'::text,
               MAX(d.delivered_at::date) FILTER (WHERE d.delivered_at < (CURRENT_DATE + 1)::timestamp),
               NULL::date, 'fnet'::text,
               'Field-level diffs of re-filed FNET documents (fund_restatement_diff), FIDC informe mensal estruturado only in slice 1. Worked as a QUEUE, newest re-filing first, capped per run, so the span has holes by design: fund_restatements.diff_status is NULL for a re-filing the queue has not reached, which is not "unchanged". A compared pair with n_fields_changed = 0 is a re-upload with nothing changed. Bodies are not stored (hashes and diffs only); re-fetch by source_url.'::text,
               MAX(d.delivered_at::date), '*fnet_diff*'::text
...
SELECT $json${
  ...
  "examples": [
    ...
    },
    {
      ...
      "then": "Each row is a re-filed document (versao > 1; modalidade RE is voluntary, RC was required by CVM) with lag_days since the version it replaced. The pairing is by a stated group key because FNET links no versions; cnpj NULL means the fortnightly fund sweep has not linked it yet — never match it to a fund by fund_name. Open the versions with fund_documents' source_url. Where diff_status = compared, fund_restatement_diff (p_fnet_id) lists exactly which fields changed."
    },
    {
      "ask": "What did this FIDC change when it re-filed its monthly informe?",
      "call": "POST /rest/v1/rpc/fund_restatement_diff {\"p_fnet_id\": <fnet_id from fund_restatements>}",
      "then": "One row per field that differs, old_value and new_value as printed (comma or dot decimals as FNET filed them), delta only when both parsed. A tranche or cedente block is addressed by its declared key; match_basis = position means the block was matched by order and the row is approximate. Slice 1 covers FIDC informe mensal only; a row with diff_status NULL in fund_restatements has not been looked at, which is not the same as unchanged."
    },
    {
