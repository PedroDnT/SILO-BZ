"""FII inf_mensal — GERAL subtype field map.
...
NOTE: the `geral` member is essentially fund-registry data repeated monthly
(ISIN, segment, administrator, address).  The architecturally cleaner move is
to route these static attributes to cvm_fund_registry (W2) and keep
cvm_fii_mensal for time-varying metrics only.  For now this map targets the
existing table; columns not present in cvm_fii_mensal fall through to residual
`raw`.
"""
